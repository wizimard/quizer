...
import type { IQuizUpdateSettingsData } from './ports/quiz.repository.port';
...
export class QuizSettings implements IQuizSettings {
	...
	static buildUpdateData(
		...
		if (periods.add?.length) {
			updateSettingsData.availablePeriods.add = periods.add.map((period) => new QuizAvailablePeriod(0, quizId, period.available_from, period.available_to));
		}

		if (periods.update?.length) {
			updateSettingsData.availablePeriods.update = periods.update.map((period) => new QuizAvailablePeriod(period.id, quizId, period.available_from, period.available_to));
		}
