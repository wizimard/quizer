...
export function validateQuestionOptions(
	options: IQuestionConfigOption[] | unknown,
	rules: QuestionOptionsValidationRules,
): IValidationError[] {
	...
	const lengthInvalid =
		(minLength !== undefined && options.length < minLength) ||
		(maxLength !== undefined && options.length > maxLength);
